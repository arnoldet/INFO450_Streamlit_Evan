# -*- coding: utf-8 -*-
"""Streamlit.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1iaChZCA9ZTsz92H_BSV56HIaYkpaq46e
"""

try:
  import streamlit as st
except ModuleNotFoundError:
  import os
  os.system("pip install streamlit")
  import streamlit as st

import pandas as pd
import plotly.express as px

df=pd.read_csv('info450_streamlit_evan/Advertising_F.csv')
df.head()

#Show admin stuff
st.title("Advertising Effectiveness Dashboard_Evan Arnold")
st.sidebar.header("Filters")
x_var = st.sidebar.selectbox("X-Axis",['TV','radio','newspaper'])
y_var = st.sidebar.selectbox("Y-Axis",['sales'])
#Show scatter plot
fig=px.scatter(df,x=x_var,y=y_var,trendline='ols')
st.plotly_chart(fig)
#Show the data frame
st.write('### Data Frame')
st.dataframe(df)
#Showing a map
st.title("I'm the MAP!!!!!")
fig=px.scatter_mapbox(df,lat='latitude',lon='longitude',color='sales',size='TV', hover_name='City')
fig.update_layout(mapbox_style='carto-positron')
st.plotly_chart(fig)
